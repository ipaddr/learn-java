...
class MyFirstThread extends Thread{
    ...
    public void run() {
        ...
        for (int i = 0; i < 20; i++) {
            try {
                Thread.sleep(500);
                System.out.println("Running from MyFirstThread on iteration : "+i);
...
class MySecondThread extends Thread{
    ...
    public void run() {
        ...
        for (int i = 0; i < 20; i++) {
            try {
                Thread.sleep(500);
                System.out.println("Running from MySecondThread on iteration : "+i);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
...
public class ThreadConcurencyExample {
    public static void main(String[] args) {
        Thread firstThread = new MyFirstThread();
        System.out.println(firstThread.getId());

        Thread secondThread = new MySecondThread();
        System.out.println(secondThread.getId());

        firstThread.start();
        secondThread.start();
    }

    private static void threadSummaryExample(){
